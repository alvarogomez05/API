...
const postImagen = async (req,res) => {
    let imagenes = await ImagenService.postImagen(req.body)
    res.send(imagenes).status(200)
};

const updateImagen = async (req,res) => {
    let imagenes = await ImagenService.updateImagen(req.params.id,req.body)
    res.send(imagenes).status(200)
};

const deleteImagen = async (req,res) => {
    let imagenes = await ImagenService.deleteImagen(req.params.id)
    res.send({
        status: 200,
        ok: true
    })
